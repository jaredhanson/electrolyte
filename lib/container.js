...
Container.prototype.specs = function() {
  var ids = Object.keys(this._spec)
    , specs = []
    , i, len;
  for (i = 0, len = ids.length; i < len; ++i) {
    specs.push(this._spec[ids[i]]);
  }
  return specs;
}
...
Container.prototype._loadSpec = function(id) {
  ...
  for (var i = 0, len = order.length; i < len; ++i) {
    ...
    if (mod) {
      this._registerSpec(id, mod, source.id);
      break;
    }
  }
}

Container.prototype._registerSpec = function(id, mod, sid) {
