...
var EventEmitter = require('events')
  ...
  , HostingContainer = require('./hostingcontainer')
...
Container.prototype.use = function(ns, fn) {
  ...
  this._sources[id] = { load: fn, namespace: ns };
  this._order.unshift(id);
  
  if (fn && fn.used) {
    var hc = new HostingContainer(this, ns, id);
    fn.used(hc);
  }
  return this;
}
 ...
 * @private
 */
Container.prototype._loadSpec = function(id, hs, sid) {
  debug('autoload %s', id);
  var order = this._order
    , source, rid, mod;
  
  if (hs !== undefined) { // fast path
    source = this._sources[hs];
    rid = path.relative(source.namespace, id);
    if (rid.indexOf('../') == 0) { return; }
    mod = source.load(rid);
    
    if (mod) {
      this._registerSpec(id, mod, hs);
    } else {
      throw new Error(sid + ' not found in source');
    }
    return;
  }
 ...
 * @private
 */
Container.prototype._registerSpec = function(id, mod, hs) {
  var spec, pattern;
  ...
  case 'factory':
    ...
    spec = new FactorySpec(id, mod, hs);
    break;
  case 'constructor':
    debug('register constructor %s', id);
    spec = new ConstructorSpec(id, mod, hs);
    break;
  case 'literal':
  default:
    debug('register literal %s', id);
    spec = new LiteralSpec(id, mod, hs);
    break;
  }
