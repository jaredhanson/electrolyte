...
Container.prototype.use = function(ns, fn, options) {
  ...
  var source = { namespace: ns, load: fn };
...
Container.prototype._registerSpec = function(id, mod, source) {
  ...
  // If the value is set to undefined, then we can check to see if it is
  // configured to be singleton by default.
  if(mod["@singleton"] === undefined && this._isSingletonByDefault) {
    mod["@singleton"] = true;
  }
...
Container.prototype.setSingletonByDefault = function(isSingletonByDefault) {
  this._isSingletonByDefault = isSingletonByDefault;
}
