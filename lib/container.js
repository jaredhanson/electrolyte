 ...
 *
 * Objects are according to a specification.  A specification defines the
 ...
 * necessary.
 *
 * The objects required by an object can be requested by interface.  The
 * container will create an object that implements the requested interface,
 * according to the runtime environment and configuration.  This is a
 * component-based approach to software development, and objects created in this
 * manner are also referred to as components.  Such an approach increases
 * modularity, allowing indepedent components to be composed into larger
 * systems.
 *
 * @constructor
 * @api public
...
Container.prototype.use = function(ns, asm) {
  ...
  for (i = 0, len = ids.length; i < len; ++i) {
    spec = asm.load(ids[i])
    this._registerSpec(ids[i], spec, asm);
  }
...
Container.prototype.create = function(id, parent) {
  var pasm = parent ? parent._source : undefined;
  
  // built-ins
  switch (id) {
  case '!container':
    return Promise.resolve(new InjectedContainer(this, parent, pasm && pasm.namespace));
  }
  ...
  return new Promise(function(resolve, reject) {
    ...
    function create(spec) {
      if (!parent || spec._assembly.h == parent._assembly.h) {
        ...
        // assembly.
        if (!spec._assembly.isExported(spec.id)) {
          return reject(new Error('Private component "' + spec.id + '" required by "' + parent.id + '"'));
        }
        return resolve(spec.create(self));
      }
    }
