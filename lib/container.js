 ...
 *
 * Objects are according to a component specification.  A component
 ...
 * graph of objects as necessary.  Objects created from a component
 * specification are also referred to as components.
 *
...
Container.prototype.use = function(ns, asm) {
  ...
  for (i = 0, len = ids.length; i < len; ++i) {
    fact = asm.load(ids[i])
    this._registerComponent(ids[i], fact, asm);
  }
 ...
 * @param {Spec} [parent] - (private) The parent component requiring the object.
 * @returns {Promise}
 * @public
 */
...
Container.prototype._loadSpec = function(id, cb) {
  ...
  for (i = 0, len = order.length; i < len; ++i) {
    ...
    if (mod) {
      spec = this._registerComponent(id, mod, source);
      return cb(null, spec);
    }
  }
