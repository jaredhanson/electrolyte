...
Container.prototype.create = function(id, parent, ecomp, options) {
  ...
  */
  ...
  if (id[id.length - 1] == ']') {
    // TODO: Parse parameters in a more structuref fashion.  Currently only used
    // for URI references.
    params = id.slice(id.indexOf('[') + 1, id.length - 1);
    ...
    // FIXME: Get all key=value params and put them as a final options argument
    if (params[params.length - 1].indexOf('=') != -1) {
      //console.log('PARSE IT!');
      
      var k, v;
      k = params[params.length - 1].slice(0, params[params.length - 1].indexOf('='));
      //console.log(k);
      v = params[params.length - 1].slice(params[params.length - 1].indexOf('=') + 1);
      //console.log(v);
      
      params[params.length - 1] = {};
      params[params.length - 1][k] = v;
    }
  ...
  if (this._variables[id]) {
    ...
    return this._variables[id].apply(this, params)
      .catch(function(err) {
