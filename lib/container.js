 ...
 * @public
 */
Container.prototype.use = function(ns, s) {
  if (typeof ns !== 'string') {
  ...
  if (typeof load != 'function') {
    throw new TypeError("Container#use requires \"asm\" to be either a function or an object with a \"load\" function, '" + (typeof load) + "' has been passed");
  }
  ...
  // TODO: Make sure all things not listed as a component are private, and requirable
  //       only within the package.
  
  for (i = 0, len = ids.length; i < len; ++i) {
    aid = path.join(source.namespace, ids[i]);
    spec = source.load(ids[i])
