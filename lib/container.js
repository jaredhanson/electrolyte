    ...
    if (comp) {
      ...
      this._loadComponent(id, function(err, comp) {
        if (err instanceof ComponentNotFoundError) {
          //reject(new ComponentNotFoundError('Cannot find component "' + id + '"'));
          
          reject(new ComponentCreateError('Unable to create object "' + id + '" required by: ' + (parent && parent.id || 'unknown')), err);
        } else if (err) {
          return reject(err);
        }
        return create(comp);
